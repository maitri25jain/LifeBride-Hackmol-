  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg-deep)' }}>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        {activeTab === 'recipients' && <RecipientsTab onStatsChange={loadStats} />}
        {activeTab === 'donors' && <DonorsTab onStatsChange={loadStats} />}
...
function RecipientsTab({ onStatsChange }) {
  ...
  async function handleCreate(e) {
    ...
    try {
      ...
      setList(fresh);
      if (onStatsChange) onStatsChange(); // refresh dashboard counts
...
function DonorsTab({ onStatsChange }) {
  const [list, setList] = useState([]);
  const [loaded, setLoaded] = useState(false);
  const [showForm, setShowForm] = useState(false);
